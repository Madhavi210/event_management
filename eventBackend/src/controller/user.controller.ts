...
export default class UserController {
  public static async createUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const newUser = await UserService.createUser(name, email, password, session);
  ...
  public static async loginUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const { userId, token } = await UserService.loginUser(email, password);
      res.status(StatusCode.OK).json({ userId, token });
    } catch (error) {
      next(error);
    }
  }
